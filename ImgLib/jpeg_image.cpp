...
void SaveImageToScanLine(vector<JSAMPLE>& buffer, int y, const Image& image) {
    const Color* iter = image.GetLine(y);
    const Color* end = iter + image.GetWidth();
    int idx = 0;
    for (; iter < end; ++iter) {
        buffer[idx++] = static_cast<JSAMPLE>(iter->r);
        buffer[idx++] = static_cast<JSAMPLE>(iter->g);
        buffer[idx++] = static_cast<JSAMPLE>(iter->b);
    }
}
...
//    jpeg_set_quality(&cinfo, quality, TRUE /* limit to baseline-JPEG values */);
    ...
    */
    ...
    vector<JSAMPLE> image_buffer(row_stride);
    while (cinfo.next_scanline < cinfo.image_height) {
        ...
        SaveImageToScanLine(image_buffer, y, image);
